...
# - tstart and tend: the range in seconds of the file to be analyzed; default to the beginning and end of file
# if not specified
...
class TonalFragment(object):
    def __init__(self, waveform, sr, tstart=None, tend=None):
        ...
        if self.t_start is not None:
            self.t_start = librosa.time_to_samples(self.t_start, sr=self.sr)
        if self.t_end is not None:
            self.t_end = librosa.time_to_samples(self.t_end, sr=self.sr)
        self.y_segment = self.waveform[self.t_start:self.t_end]
    ...
    def compute_key(self):
        ...
        chroma_vals = []
        for i in range(12):
            chroma_vals.append(np.sum(self.chromograph[i]))
