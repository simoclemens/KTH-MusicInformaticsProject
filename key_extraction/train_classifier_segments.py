...
from models import KeyClassifier
import datetime
import sys
import numpy as np
...
def train(file, net, train_loader, val_loader, optimizer, cost_function, n_classes, batch_size=1,
    ...
    for iteration in range(training_iterations):
        ...
        n_segments = data_source['features'].shape[0]
        for segment in range(n_segments):
            inputs = data_source['RGB'][:, segment].to(device)
        ...
        test_metrics = validate(net, val_loader, n_classes, n_segments, batch_size)
...
# validation function
def validate(net, val_loader, n_classes, n_clips=5, batch_size=1, device="cuda:0"):
    ...
    with torch.no_grad():  # do not update the gradient
        for iteration, (data_source) in enumerate(val_loader):  # extract batches from the val_loader
            ...
            for segment in range(n_segments):
                ...
                inputs = data_source[:, segment].to(device)
